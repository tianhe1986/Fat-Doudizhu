...
var Game;
(function (Game) {
    var PokerLogic = /** @class */ (function () {
        function PokerLogic() {
        }
        //是否可出
        PokerLogic.prototype.canOut = function (nowPokers, myReadyPokers) {
            //如果当前没有牌，只要牌型正确，就可以出
        ...
        //计算牌型
        PokerLogic.prototype.calcuPokerType = function (cards) {
            //转换为点数
            var points = this.cardsToPoints(cards);
            ...
            if (len == 1) { //单牌
                ...
                var maxSameNum = this.calcMaxSameNum(points);
                var diffNum = this.calcDiffNum(points);
                if (len % 3 == 0 && maxSameNum == 3 && diffNum == len / 3 && (points[len - 1] - points[0] == len / 3 - 1) && points[len - 1] != 15) { //三张牌飞机不带
                    ...
                    var threeCards = this.calcSameNumCards(points, 3);
        ...
        //计算最大的牌
        PokerLogic.prototype.calcPokerHeader = function (cards, type) {
            var points = this.cardsToPoints(cards);
            switch (type) {
        ...
        //最多有多少张点数相同的牌
        PokerLogic.prototype.calcMaxSameNum = function (points) {
            var maxNum = 1;
            var nowNum = 1;
        ...
        //第一个出现N次的点数
        PokerLogic.prototype.calcFirstPoint = function (points, num) {
            var nowNum = 1;
            for (var i = 0; i < points.length - 1; i++) {
                if (points[i] == points[i + 1]) { //与下一张相同，数量加1
                    nowNum++;
        ...
        PokerLogic.prototype.calcSameNumCards = function (points, num) {
            var cards = new Array();
            var nowNum = 1;
            for (var i = 0; i < points.length - 1; i++) {
                if (points[i] == points[i + 1]) { //与下一张相同，数量加1
        ...
        //有多少不同的点数
        PokerLogic.prototype.calcDiffNum = function (points) {
            var diffNum = 1;
            for (var i = 0; i < points.length - 1; i++) {
        ...
        //是否是顺子
        PokerLogic.prototype.isStraight = function (points) {
            for (var i = 0; i < points.length - 1; i++) {
        ...
        //牌转点数
        PokerLogic.prototype.cardsToPoints = function (cards) {
    ...
    Game.PokerLogic = PokerLogic;
})(Game || (Game = {}));
//# sourceMappingURL=PokerLogic.js.map